...
exports.createResena = async (data, ctx) => {
  ...
  try {
    ...
    const { rows: propRows } = await client.query(
      `SELECT (co.usuario_id = $2) AS es_propio
       FROM inmuebles i
    ...
    if (!propRows.length) throw new AppError("El inmueble no existe", 404);
    if (propRows[0].es_propio) {
      throw new AppError("No puedes escribir una reseña en tu propio inmueble", 403);
    }

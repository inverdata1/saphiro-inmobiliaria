...
export default function RegistroCorredorPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await apiGet(`/auth/registro-token/corredor/${token}`);
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Este enlace es inválido o ha expirado. Solicita un nuevo registro de corredor para recibir otro enlace.
          </p>
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Corredor: <span className="font-semibold text-slate-700 dark:text-slate-300">{email}</span>
          </div>
        </div>
